...
def get_manifest():
    ...
    try:
        ...
        # Buscamos el mejor manifiesto disponible
        for f in info.get('formats', []):
            if f.get('protocol') == 'm3u8_native' or f.get('protocol') == 'm3u8':
                manifest_url = f.get('url')
                break # Preferimos HLS
            elif f.get('protocol') == 'mpd':
                manifest_url = f.get('url')
        
        if not manifest_url:
             # Plan B: Si no hay manifiesto, buscar un enlace MP4 directo
            for f in info.get('formats', []):
                if f.get('ext') == 'mp4' and f.get('vcodec') != 'none':
                    manifest_url = f.get('url')
                    return jsonify({ "manifest_url": manifest_url, "title": title, "is_direct": True })
